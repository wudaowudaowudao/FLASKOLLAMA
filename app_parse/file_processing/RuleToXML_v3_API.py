...
# 设置智谱AI API密钥
# os.environ["ZHIPUAI_API_KEY"] = "d6366483a4ae45fb9a4be838eb222ee7.jr7A8GlghBmdbkgM"
os.environ["ZHIPUAI_API_KEY"] = "d6366483a4ae45fb9a4be838eb222ee7.jr7A8GlghBmdbkgM"

# 配置日志
class RuleToXMLConverter:
    def __init__(self, model_name: str = "glm-4-air-250414", temperature: float = 0.1):
        """初始化规则转XML转换器"""
    ...
    def create_llm(self) -> ChatZhipuAI:
        """创建智谱AI LLM实例"""
        return ChatZhipuAI(
...
"""
    ...
    def parse_result(self, result: str) -> str:
        ...
        spec_match = re.search(r'<SpecificationCheck>([\s\S]*?)</SpecificationCheck>', result)
        if spec_match:
            result = f"<SpecificationCheck>{spec_match.group(1)}</SpecificationCheck>"
