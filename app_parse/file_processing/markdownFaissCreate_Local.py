...
class MarkdownFaissCreator:
    ...
    def load_markdown_data(self, file_path):
        """
        读取 Markdown 文件内容
    ...
    def split_markdown_text(self, text, split_enter_count=5, overlap=1):
        """
        按指定数量分割 Markdown 文本为多个 chunk，并设置重叠部分
    ...
    def create_faiss(self, markdown_file_path, save_path):
        """
        主方法：从 Markdown 文件创建 FAISS 向量存储
        ...
        """
        # 加载 Markdown 数据
        markdown_text = self.load_markdown_data(markdown_file_path)
        print('len of markdown_text: ', len(markdown_text))

        # 分割文本为 chunk
        chunks = self.split_markdown_text(markdown_text)
...
# 主程序
def main():
    markdown_file_path = 'E:\\PycharmProjects\\FlaskOllama\\data\\trans3_zh.md'
    save_path = "E:\\PycharmProjects\\FlaskOllama\\data\\vector_md_db"
    
    # 创建实例并运行
    creator = MarkdownFaissCreator()
    creator.create_faiss(markdown_file_path, save_path)
